 ...
 * - Timeline visuelle à gauche (desktop)
 * - Cartes profondes avec icône, durée indicative, livrables clés
 * - Détails repliables par étape (toggle)
 * - Mini-métriques en bas (démarrage, rituel, focus, outillage)
 */
...
function Badge({children}){
  return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] bg-gray-100 text-gray-700 border">{children}</span>;
}
...
function StepCard({step}){
  ...
  return (
    <div className="relative card p-6 transition-all hover:shadow-lg">
      <div className="absolute -left-2 top-6 w-5 h-5 rounded-full bg-white border shadow-sm flex items-center justify-center text-[11px] font-semibold">{step.n}</div>
          ...
          <div className="mt-2 rounded-2xl border p-3 bg-white text-sm text-gray-700">
            {step.n === 1 && (
              <div>
                ...
                <p className="mt-1">Segments: SaaS B2B 1–50 salariés, éditeurs CRM/ERP avec 500–3k clients. Personas: CEO, Head of Product, Partnerships.</p>
              </div>
            )}
            {step.n === 2 && (
              <div>
                <div className="font-medium">Exemple — Script email (extrait)</div>
                <p className="mt-1">Objet: “Intégrer une brique facturation sans refondre votre produit ?” — 4 lignes, CTA 20 min, variante A/B objet + PS.</p>
              </div>
            )}
            {step.n === 3 && (
              <div>
                <div className="font-medium">Exemple — Playbook</div>
                <p className="mt-1">Checklist: domaines, tracking, variables, segments, mapping pipeline, critères de qualif, rituel hebdo.</p>
              </div>
            )}
            {step.n === 4 && (
              <div>
                <div className="font-medium">Exemple — Qualification RDV</div>
                <p className="mt-1">Critères: budget présent/approvable, timing < 90j, sponsor identifié, besoin reconnu, décisionneur mappé.</p>
              </div>
            )}
            {step.n === 5 && (
              <div>
                <div className="font-medium">Exemple — Learning A/B</div>
                <p className="mt-1">Objet court + bénéfice métier > objet technique. Relance J+2 performe mieux que J+1 sur segment CFO.</p>
              </div>
            )}
          </div>
        ) : null}
